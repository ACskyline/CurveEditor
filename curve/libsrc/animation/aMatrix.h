// SvzMatrix.h
// Author: Stephen Lane, Joey Schnurr 
// Copyright (c) 2010 - 2011 soVoz Inc.  All Rights Reserved
//////////////////////////////////////////////////////////////////////
...
// tested with this class are SvzValue, double, long double, complex<SvzValue>,
...
//                  generate a different type of matrix like SvzValue.
//
//
...
//
//  SHL ->  This is the version to use
#ifndef __cplusplus
#error Must use C++ for the type matrix.
#endif

#if !defined(_TCL_MATRIX_H)
#define _TCL_MATRIX_H
...
#define _NO_EXCEPTION   // by defining we will not be using exceptions in the application

// _NO_NAMESPACE - by not defining will be using namespaces in the application
// _NO_TEMPLATE  - by not defining will be using template classes in the application
//////////////////////////////////////////////////////////////////////////////////////////// 
...
#ifndef _NO_NAMESPACE
   namespace math {			// all in context of math namespace
#endif
...
//***SHL mod -  change these defines to affect default size of matrix pointed to by ValArray and ValMat
#define FULLSTATE
...
//   bool operator != (const matrixT& m1, const matrixT& m2) _NO_THROW;
   ...
   T Cond () _NO_THROW;
   T* ValOut() _NO_THROW;
   void ValIn(T* mat) _NO_THROW;
   void ValIn2(T* mat) _NO_THROW;
...
private:
	...
	T **Val;
	T *ValArray;  								// ***SHL mod
	typedef T dim2[MATDIM2];
	typedef dim2* dim1xdim2[MATDIM1];			// ***SHL mod
	dim1xdim2 *ValMat;							// ***SHL mod
	...
	base_mat (size_t row, size_t col, T** v)
	{
	    ...
	    Val = new T* [row];
		ValArray = new T[row*col];				// ***SHL mod
		ValMat = (dim1xdim2*) Val;				// ***SHL mod
...
//  ***SHL additions
// set elements of this matrix to valArray
MAT_TEMPLATE inline void
matrixT::ValIn(T *mat) _NO_THROW
{
	if (_m->Refcnt > 1) clone();   
	for (size_t i=0; i < _m->Row; i++)
		for (size_t j=0; j < _m->Col; j++) {
			_m->Val[i][j] = mat[i*_m->Col+j];
		}
	return;
}

// set elements of this matrix and valArray to contents of mat
MAT_TEMPLATE inline void
matrixT::ValIn2(T *mat) _NO_THROW
{
	...
	for (size_t i=0; i < _m->Row; i++)
		for (size_t j=0; j < _m->Col; j++) {
			_m->Val[i][j] = mat[i*_m->Col+j];
			_m->ValArray[i*_m->Col+j] = _m->Val[i][j];
		}
		return;
}


// set elements of this matrix to ValArray and return pointer to valArray
MAT_TEMPLATE inline T*
matrixT::ValOut() _NO_THROW
{
	if (_m->Refcnt > 1) clone();   
	for (size_t i=0; i < _m->Row; i++)
		for (size_t j=0; j < _m->Col; j++)
