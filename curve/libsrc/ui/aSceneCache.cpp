...
VBOMesh::VBOMesh() : mHasNormal(false), mHasUV(false), mAllByControlPoint(true), mNormals(0)
{
...
bool VBOMesh::Initialize(const FbxMesh *pMesh)
{
    ...
    float* lNormals = new float[lPolygonVertexCount * NORMAL_STRIDE];
    if (mNormals) delete[] mNormals; // shouldn't need this I think
    mNormals = NULL;
    if (mHasNormal)
    {
        mNormals = new FbxVector4[pMesh->GetControlPointsCount()];
        mNumNormals = pMesh->GetControlPointsCount();
    }
    float * lUVs = NULL;
    ...
    if (mAllByControlPoint)
    {
        ...
        for (int lIndex = 0; lIndex < lPolygonVertexCount; ++lIndex)
        {
            ...
            if (mHasNormal)
            {
                ...
                mNormals[lNormalIndex] = lCurrentNormal;
        ...
        for (int lVerticeIndex = 0; lVerticeIndex < TRIANGLE_VERTEX_COUNT; ++lVerticeIndex)
        {
            ...
            else
            {
                ...
                if (mHasNormal)
                {
                    ...
                    mNormals[lControlPointIndex] = lCurrentNormal;
...
void VBOMesh::UpdateVertexPosition(const FbxMesh * pMesh, const FbxVector4 * pVertices, const FbxVector4* pNormals) const
{
    ...
    if (mAllByControlPoint)
    {
        lVertexTotal = pMesh->GetControlPointsCount();
        lVertices = new float[lVertexTotal * VERTEX_STRIDE];
        for (int lIndex = 0; lIndex < lVertexTotal; ++lIndex)
        {
    ...
    {
        const int lPolygonCount = pMesh->GetPolygonCount();
        lVertexTotal = lPolygonCount * TRIANGLE_VERTEX_COUNT;
        lVertices = new float[lVertexTotal * VERTEX_STRIDE];
        if (mHasNormal && pNormals) lNormals = new float[lVertexTotal * NORMAL_STRIDE];
        ...
        for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex)
        {
            for (int lVerticeIndex = 0; lVerticeIndex < TRIANGLE_VERTEX_COUNT; ++lVerticeIndex)
            {
                ...
                if (pNormals)
                {
                    lNormals[lVertexCount * NORMAL_STRIDE] = static_cast<float>(pNormals[lControlPointIndex][0]);
                    lNormals[lVertexCount * NORMAL_STRIDE + 1] = static_cast<float>(pNormals[lControlPointIndex][1]);
                    lNormals[lVertexCount * NORMAL_STRIDE + 2] = static_cast<float>(pNormals[lControlPointIndex][2]);
                }
                ++lVertexCount;
            }
        }
    ...
    {
        glBindBuffer(GL_ARRAY_BUFFER, mVBONames[VERTEX_VBO]);
        glBufferData(GL_ARRAY_BUFFER, lVertexTotal * VERTEX_STRIDE * sizeof(float), lVertices, GL_STATIC_DRAW);
        delete [] lVertices;
    }
    ...
    {
        glBindBuffer(GL_ARRAY_BUFFER, mVBONames[NORMAL_VBO]);
        glBufferData(GL_ARRAY_BUFFER, lVertexTotal * NORMAL_STRIDE * sizeof(float), lNormals, GL_STATIC_DRAW);
        delete[] lNormals;
    }
}
...
void VBOMesh::Draw(int pMaterialIndex, ShadingMode pShadingMode) const
{
    ...
    if ( pShadingMode == SHADING_MODE_SHADED || pShadingMode == SHADING_MODE_FLAT)
    {
...
void VBOMesh::BeginDraw(ShadingMode pShadingMode) const
{
    ...
    else if (pShadingMode == SHADING_MODE_FLAT)
    {
        glColor4f(0,0,0,1);
    }
    else
...
FbxDrawText::FbxDrawText() : mGlyph(NULL), mTextureName(0), mPointSize(DEFAULT_POINT_SIZE), mGap(DEFAULT_GAP)
{
    Initialize();
}
