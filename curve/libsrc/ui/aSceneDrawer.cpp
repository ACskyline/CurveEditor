              ...
              FbxPose* pPose, ShadingMode pShadingMode)
{
    ...
    if (lNodeAttribute)
    {
        ...
        {
            if (AFBX::gDrawMesh) DrawMesh(pNode, pTime, pAnimLayer, pGlobalPosition, pPose, pShadingMode);
        }
              ...
              FbxAMatrix& pGlobalPosition, FbxPose* pPose, ShadingMode pShadingMode)
{
    ...
    if (!lMeshCache || lHasDeformation)
    {
        ...
        if (lMesh && lMesh->GetUserDataPtr())
        {
            ...
            memcpy(lNormalArray, lMeshCache->mNormals, lVertexCount * sizeof(FbxVector4));
        }
    }
    ...
    if (lHasDeformation)
    {
        ...
        if (lMeshCache)
            lMeshCache->UpdateVertexPosition(lMesh, lVertexArray, lNormalArray);
    }
    ...
    if (lMeshCache)
    {
        ...
        if (lNormalArray)
        {
            if (lMesh && lMesh->GetUserDataPtr())
            {
                ...
                glBegin(GL_LINES);
                for (int i = 0; i < lMeshCache->mNumNormals; i++)
                {
                    float nx = lNormalArray[i * 3 + 0];
                    float ny = lNormalArray[i * 3 + 1];
                    float nz = lNormalArray[i * 3 + 2];
                    float x = lVertexArray[i][0];
                    float y = lVertexArray[i][1];
                    float z = lVertexArray[i][2];
                    glVertex3f(x, y, z);
							   ...
							   FbxPose* pPose)
{
	...
	for (int i = 0; i < lVertexCount; i++) 
	{
		...
		if (lWeight != 0.0) 
		{
            ...
            FbxAMatrix tmp = lClusterDeformation[i].Transpose().Inverse();
									 ...
									 FbxPose* pPose)
{
	...
	{
		ComputeLinearDeformation(pGlobalPosition, pMesh, pTime, pVertexArray, pNormalArray, pPose);
	}
	...
	else if(lSkinningType == FbxSkin::eBlend)
	{
		...
		ComputeLinearDeformation(pGlobalPosition, pMesh, pTime, lVertexArrayLinear, 0, pPose);
